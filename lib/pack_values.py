...
def pack(cols, index_vals = None, subscript_names = None, in_table = False, node=None, fp=None):
	...
	# if compound type (more than one column) then subscript_names must be present, otherwise not
	assert (len(cols) > 1 and subscript_names) or (len(cols) == 1 and subscript_names is None)
	packed_columns = []
	column_types = []
	for col in cols:
	...
	if index_vals:
		...
		index_str = ""
	if subscript_names:
		# subscript names given, form column with subscript names and column type
		assert len(subscript_names) == len(column_types)
		ssc_str = delimiter.join( [ escape(subscript_names[i] 
...
def unpack(packed, value_type, required_subscripts=None):
	# returns dictionary like: {
	# 'cols': [ [col1], [col2], ...]
	# 'subscripts': [ subscript_names, ],
	# 'column_types': [ type_code, ...]
	# 'index_values': [... ]
	# }
	# or None if any required_subscripts are not present
	uv = { }  # dict for storing unpacked values

	def extract_subscript_names(col):
	...
	if value_type == 'c':
		...
		assert len(cols) > 1
		extract_subscript_names(cols[0])
		cols = cols[1:]
...
def run_tests():
	...
	subscript_names = ["name1", "weight2", "age", "bday-3'rd", "city,4" ]
	cols = [
	...
	print("subscript_names: %s" % subscript_names)
	pp.pprint(cols)
	for col in cols:
		vp.add_column( col )
	print("index_vals: %s" % index_vals)
	vp.add_index_vals( index_vals )
	vp.add_subscript_names(subscript_names)
	packed_values = vp.get_packed_values()
	print("packed values are:")
	print(packed_values)

	# try packing with function
	pv2, type_code = pack(cols, index_vals=index_vals, subscript_names=subscript_names, in_table=True)
